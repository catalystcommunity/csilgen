...
use csilgen_common::{
    wasm_interface::*, CsilFieldMetadata, CsilFieldVisibility, CsilGroupKey, CsilRuleType,
...
fn generate_types_documentation(
    content: &mut String,
                        ...
                        if let Some(key) = &entry.key {
                            ...
                            if description == "*No description*" {
                                warnings.push(GeneratorWarning {
                                    ...
                                    message: format!(
                                        "Field '{}' in type '{}' has no description",
                                        field_name, rule.name
                                    ),
                                    location: None,
                                    suggestion: Some(
                                        "Add @description annotation for better documentation"
                                            .to_string(),
                                    ),
                                });
                            }
...
fn generate_services_documentation(
    content: &mut String,
    input: &WasmGeneratorInput,
...
fn format_type_for_docs(type_expr: &CsilTypeExpression) -> String {
    match type_expr {
        ...
        CsilTypeExpression::Map { key, value, .. } => {
            format!(
                "map<{}, {}>",
                format_type_for_docs(key),
                format_type_for_docs(value)
            )
        }
