...
/// The package README, with a copy-paste **Quickstart**. For a spec with service
/// operations the Quickstart is a complete, dependency-free CSIL-RPC carrier — it
/// reuses this package's own generated CBOR codec (`cbor_encode` / `cbor_decode` /
/// `CborTag`) for the envelope, so it adds no third-party dependency — the typed
/// sync client constructed over it, and one example call. A serviceless spec gets a
/// shorter "import the types + codec" section instead.
fn readme(spec: &CsilSpecSerialized, records: &HashSet<String>, pkg: &PythonPackage) -> String {
    let dist = &pkg.dist_name;
    let import = &pkg.import_name;
    let mut out = format!(
        "# {dist}\n\n\
         Generated by csilgen. A typed CSIL-RPC client: the generated codec owns CBOR\n\
         (de)serialization; you supply a *carrier* that only moves bytes.\n\n\
         ## Install\n\n\
         ```sh\n\
         pip install {dist}\n\
         ```\n\n\
         <!-- TODO: replace with the published artifact once it ships. -->\n\n"
    );

    match first_unary_example(spec, records) {
        Some(example) => out.push_str(&quickstart(import, &example)),
        None => out.push_str(&format!(
            "## Quickstart\n\n\
             This package has no service operations — import its generated dataclasses\n\
             and codec directly:\n\n\
             ```python\n\
             from {import} import *  # dataclasses + cbor_encode / cbor_decode\n\
             ```\n"
        )),
    }
    out
...
/// The client Quickstart: the dependency-free blocking CSIL-RPC carrier, then the
/// typed sync client constructed over it, then one example call. Python's stdlib HTTP
/// (`urllib`) is naturally blocking, so the sync client is the right surface here.
fn quickstart(import: &str, ex: &UnaryExample) -> String {
    // Imports: the typed client + `ServiceError`, the request (and nested record)
    // types the sample literal constructs, and exactly the codec primitives the
    // carrier uses to build/parse the CSIL-RPC envelope.
    let mut names = vec![ex.client_class.clone(), "ServiceError".to_string()];
    names.extend(ex.used_types.iter().cloned());
    names.extend(["CborTag", "cbor_decode", "cbor_encode"].map(str::to_string));
    ...
    out.push_str(CARRIER_PYTHON);
    out.push_str("\n\ndef main() -> None:\n");
    out.push_str(&format!(
        "    client = {}(CsilRpcTransport(\"http://localhost:5080\"))\n",
        ex.client_class
    ));
    if ex.has_request {
...
/// The carrier body — identical for every spec, so it is a constant. Hybrid posture
/// path 1: it reuses the generated codec's exported generic CBOR (`cbor_encode` /
/// `cbor_decode` / `CborTag`, imported by the snippet above) to build the CSIL-RPC
/// envelope, so it needs NO third-party dependency. It wraps the already-encoded
/// request in tag 24, POSTs it to `{base_url}/csil/v1/rpc` with blocking `urllib`,
/// and unwraps the response payload — surfacing a non-zero `status` or a typed
/// `ServiceError` arm as an exception.
const CARRIER_PYTHON: &str = r#"# Carrier path: reuse the generated codec's exported generic CBOR (cbor_encode /
# cbor_decode / CborTag) to build the CSIL-RPC envelope, so this carrier needs NO
# third-party dependency.
import urllib.error
import urllib.request

TAG_ENCODED_CBOR = 24  # RFC 8949 embedded-CBOR tag


class TransportError(Exception):
    """A CSIL-RPC transport-level failure (non-zero status)."""

    def __init__(self, code: int, message: str):
        self.code = code
        self.message = message
        super().__init__(f"transport status {code}: {message}")
...
class CsilRpcTransport:
    ...
    def call(self, service: str, op: str, req: bytes) -> bytes:
        # CsilRpcRequest = { v, service, op, payload: #6.24(bstr) }: the already-encoded
        # request bytes wrapped in tag 24 (embedded CBOR).
        envelope = cbor_encode(
            {"v": 1, "service": service, "op": op, "payload": CborTag(TAG_ENCODED_CBOR, req)}
        )
        ...
        try:
            ...
            raise TransportError(exc.code, f"http {exc.code}") from None

        env = cbor_decode(raw)
        status = int(env.get("status", 0))
        if status != 0:
            raise TransportError(status, str(env.get("error", "")))
        payload = env.get("payload")
        inner = payload.value if isinstance(payload, CborTag) else payload
        # A typed `ServiceError` arm is an application error, distinct from a transport
        # failure; decode its {code, message} map and raise the generated exception.
        if env.get("variant") == "ServiceError":
            err = cbor_decode(inner)
            raise ServiceError(int(err["code"]), str(err["message"]))
        return inner
"#;

/// The pieces the Quickstart's example call needs: which client class + method to
/// call, a constructible sample request literal (empty when the op takes no request),
/// and the record type names the literal constructs (so the snippet imports them).
struct UnaryExample {
    client_class: String,
    method: String,
    has_request: bool,
...
impl PythonGenerator {
    ...
    fn generate(&mut self, spec: &CsilSpecSerialized) -> Result<GeneratedFiles> {
        ...
        for rule in &spec.rules {
            match &rule.rule_type {
                ...
                CsilRuleType::ServiceDef(service) => match &surface {
                    Surface::TypesOnly => {}
                    // The client surface may emit two files (a sync client and its
                    // async twin), so it is built after this loop from `client_style`
                    // rather than accumulated into the single `services_code` buffer.
                    Surface::Client => {}
                    Surface::Server => {
                        if !prelude_emitted {
                            services_code
        ...
        if matches!(surface, Surface::Client) {
        ...
        if let Some(pkg) = self.resolve_package_layout() {
            ...
            let readme = readme(spec, &records, &pkg);
...
"#;
    ...
    fn pyproject_emitted_iff_emit_packages_includes_python() {
        ...
        // files (`pyproject.toml`, `README.md`) are left beside it.
        for f in &files {
            if f.path == "pyproject.toml" || f.path == "README.md" {
                continue;
            }
            assert!(
        ...
        let readme = files
            ...
            .expect("README.md emitted at dist root in package mode");
        assert!(readme.content.contains("# acme-corndogs"));
        assert!(readme.content.contains("## Quickstart"));
    }
...
"#;
    ...
    fn pingpong_package_files(name: &str, version: &str) -> Vec<GeneratedFile> {
        let mut cfg = create_test_config(false);
        cfg.target = "python-client".to_string();
        cfg.options
            .insert("emit_packages".to_string(), serde_json::json!(["python"]));
        cfg.options
            .insert("package_name".to_string(), serde_json::json!(name));
        cfg.options
            .insert("package_version".to_string(), serde_json::json!(version));
        generate_python_code_from_serialized(&pingpong_spec(), &cfg).unwrap()
    }

    /// The README carries a copy-paste CSIL-RPC Quickstart: the seam-shaped carrier, the
    /// `/csil/v1/rpc` POST, the tag-24 payload, the `status`/`ServiceError` handling, and
    /// the typed example call with a constructible sample literal.
    #[test]
    fn package_readme_has_quickstart_carrier_and_example() {
        let files = pingpong_package_files("pingpong-sdk", "0.1.0");
        let readme = files
            .iter()
            .find(|f| f.path == "README.md")
        ...
        assert!(readme.contains("# pingpong-sdk"));
        assert!(readme.contains("pip install pingpong-sdk"));
        assert!(readme.contains("## Quickstart"));
        // The carrier imports exactly the typed client, ServiceError, the request type,
        // and the generic codec primitives it builds the envelope from.
        assert!(readme.contains(
            "from pingpong_sdk import PingClient, ServiceError, Ping, CborTag, cbor_decode, cbor_encode"
        ));
        // The seam: envelope + blocking HTTP only.
        assert!(readme.contains("class CsilRpcTransport:"));
        assert!(readme.contains("def call(self, service: str, op: str, req: bytes) -> bytes:"));
        assert!(readme.contains("/csil/v1/rpc"));
        assert!(readme.contains("CborTag(TAG_ENCODED_CBOR, req)"));
        assert!(readme.contains("urllib.request.urlopen("));
        // Both arms of the response contract.
        assert!(readme.contains("if status != 0:"));
        assert!(readme.contains("if env.get(\"variant\") == \"ServiceError\":"));
        // Path 1 (dep-free) is stated.
        assert!(readme.contains("no third-party deps"));
        assert!(readme.contains("reuse the generated codec's exported generic CBOR"));
        // The example call: typed client over the carrier with a constructible sample.
        assert!(readme.contains("PingClient(CsilRpcTransport(\"http://localhost:5080\"))"));
        assert!(readme.contains("client.ping(Ping(message=\"example\", count=0))"));
    }

    /// Drive the README's own carrier code against the real generated client + codec
    /// with a hermetic, socket-free echo: monkeypatch `urllib.request.urlopen` to decode
    /// the request envelope and echo the tag-24 inner payload in a `status:0` response,
    /// then call the typed client and assert the field round-trips. Skips cleanly when
    /// python3 is absent.
    #[test]
    fn readme_quickstart_round_trips_through_python() {
        let have = std::process::Command::new("python3")
            .arg("--version")
            .output()
            .is_ok();
        ...
        let readme = files
            ...
            .content
            .clone();
        // Extract the README's `python` Quickstart block verbatim — the carrier code we
        // actually run, so the test proves the published snippet, not a reimplementation.
        let block = readme
            .split_once("```python\n")
            .and_then(|(_, rest)| rest.split_once("\n```"))
            .map(|(code, _)| code.to_string())
            .expect("README has a python Quickstart code block");

        let dir =
            std::env::temp_dir().join(format!("csilgen-python-readme-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        for f in &files {
            let dest = dir.join(&f.path);
            std::fs::create_dir_all(dest.parent().unwrap()).unwrap();
        ...
        // The README block is itself an importable module: it defines `CsilRpcTransport`
        // and a guarded `main()` that does not run on import.
        std::fs::write(dir.join("quickstart.py"), &block).unwrap();
        std::fs::write(dir.join("driver.py"), README_ROUND_TRIP_DRIVER_PYTHON).unwrap();
        ...
        assert!(
            run.status.success(),
            "python README round-trip failed:\n{}{}",
            String::from_utf8_lossy(&run.stdout),
            String::from_utf8_lossy(&run.stderr)
        );
        assert_eq!(String::from_utf8_lossy(&run.stdout).trim(), "ok");
    ...
    const README_ROUND_TRIP_DRIVER_PYTHON: &str = r#"import os
import sys
...
from pingpong_sdk import Ping, PingClient
from pingpong_sdk.codec import CborTag, cbor_decode, cbor_encode

import quickstart  # defines the README's CsilRpcTransport carrier
...
def _echo_urlopen(req, timeout=None):
    # Hermetic, socket-free stand-in for the server: decode the CSIL-RPC request
    # envelope and echo the tag-24 inner payload back in a status:0 response.
    env = cbor_decode(req.data)
    assert env["service"] == "ping", env
    assert env["op"] == "Ping", env
    payload = env["payload"]
    inner = payload.value if isinstance(payload, CborTag) else payload
    body = cbor_encode({"v": 1, "status": 0, "payload": CborTag(24, inner)})
    return _FakeResp(body)


urllib.request.urlopen = _echo_urlopen

client = PingClient(quickstart.CsilRpcTransport("http://unused.invalid"))
resp = client.ping(Ping(message="hello", count=7))
assert resp.message == "hello", resp
assert resp.count == 7, resp
print("ok")
"#;
}
