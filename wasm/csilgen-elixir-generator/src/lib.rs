...
/// `:httpc`. The typed client decodes the success payload; a non-zero transport status
...
fn rpc_section(config: &ElixirConfig, ex: Option<&UnaryExample>) -> String {
    ...
    out.push_str(
        "Request/response. The library owns the envelope (`Csilgen.Transport.RPC`\n\
         request/response); you bring a carrier that moves bytes. The `:httpc` carrier\n\
...
/// `{base}/csil/v1/rpc` over `:httpc`, and decodes the reply with `RPC.decode_response`
...
fn rpc_carrier_elixir(config: &ElixirConfig) -> String {
  ...
  # --- HTTP over :httpc (Erlang/OTP :inets + :ssl; one example stdlib client) ---
  defp post(url, body) do
    :inets.start()
    :ssl.start()

    request = {{String.to_charlist(url), [], ~c"application/cbor", body}}

    case :httpc.request(:post, request, [], body_format: :binary) do
      {{:ok, {{{{_, code, _}}, _headers, resp_body}}}} when code in 200..299 ->
        {{:ok, resp_body}}
