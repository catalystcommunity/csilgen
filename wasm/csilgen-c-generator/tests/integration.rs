//! Integration sanity checks for the C generator. Like the Go generator's
//! external tests, these only build a `WasmGeneratorInput` and assert on the AST
//! structure (the private codegen functions are exercised by the in-module
...
fn input_round_trips_through_serde() {
    ...
    assert_eq!(back.config.target, "c");
}
