...
#[cfg(test)]
mod codec_security_tests {
    use super::*;

    #[test]
    fn decoder_rejects_bad_heads_depth_and_collection_counts() {
        assert!(CSIL_CBOR_DART.contains("depth > 64"));
        assert!(CSIL_CBOR_DART.contains("truncated argument"));
        assert!(CSIL_CBOR_DART.contains("array length exceeds remaining input"));
        assert!(CSIL_CBOR_DART.contains("utf8.decode"));
    }
}
